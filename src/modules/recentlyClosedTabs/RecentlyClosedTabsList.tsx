// src/modules/recentlyClosedTabs/RecentlyClosedTabsList.tsx
...
export const RecentlyClosedTabsList: React.FC = () => {
  ...
  const loadData = useCallback(async (currentSettings: AppSettings) => {
    const recentlyClosed = await ChromeService.getRecentlyClosedTabs(currentSettings.recentlyClosedTabs.maxTabs);
    setSessions(recentlyClosed);
  }, []);

  const loadSettings = useCallback(async () => {
    const newSettings = await DataService.getSettings();
  ...
  useEffect(() => {
    loadSettings();
    const storageListener = () => loadSettings();
  ...
  return (
    <Box>
      ...
      <Divider />
      <List dense sx={{ p: 1, maxHeight: '200px', overflowY: 'auto' }}>
        {sessions.map((session) => {
          const tab = session.tab;
          if (!tab || !tab.url) return null;
          return (
            <ListItemButton key={`${tab.sessionId}-${session.lastModified}`} onClick={() => handleRestoreTab(tab.sessionId)} sx={{ minHeight: '40px', '&:hover .actions': { opacity: 1, pointerEvents: 'auto', } }}>
              ...
              <ListItemText primary={tab.title} primaryTypographyProps={{ noWrap: true, overflow: 'hidden', textOverflow: 'ellipsis' }} sx={{ flexGrow: 1, pr: 1, minWidth: 0 }} />
              {settings.recentlyClosedTabs.showTime && <Typography variant="caption" color="text.secondary" sx={{ pr: 1 }}>{getTimeAgo(session.lastModified)}</Typography>}
              <Box className="actions" sx={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 0.5, opacity: 0, pointerEvents: 'none', transition: 'opacity 0.2s' }}>
                ...
                <Tooltip title={t('restoreTab')}><IconButton size="small" onClick={(e) => { e.stopPropagation(); handleRestoreTab(tab.sessionId); }}><RestoreIcon fontSize="small" /></IconButton></Tooltip>
              </Box>
            </ListItemButton>
          )
        })}
